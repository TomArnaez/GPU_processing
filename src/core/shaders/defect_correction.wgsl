...
fn average_neighbours(x: i32, y: i32, dimensions: vec2<i32>) -> u32 {
    var total = u32(0);
    var count = u32(0);

    for (var i = 0; i < 8; i++) {
        var neighbour_pos = vec2<i32>(x, y) + get_offset(u32(i));

        let in_bounds = u32(is_inside_bounds(neighbour_pos, dimensions));

        let neighbour_defect = textureLoad(defect_map, neighbour_pos).r;
        let neighbour_pixel = textureLoad(input_image, neighbour_pos).r;

        total += in_bounds * (1u - neighbour_defect) * neighbour_pixel;
        count += in_bounds * (1u - neighbour_defect);
    }
...
fn correct_image(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let neighbour_average = average_neighbours(x, y, dimensions);
